...
def binary_train(X, y, w0=None, b0=None, step_size=0.5, max_iterations=1000):
    ...
    for i in range(max_iterations):
        error = sigmoid(np.dot (w ,X.T)) - y
        grad = np.multiply(error, X.T)/N
        grad = grad.sum(axis=1, dtype='float')
...
def multinomial_train(X, y, C, 
    ...
    for i in range(1000):
        for j in range (N):
            prod = np.dot (X[j] , w.T)
            total = np.exp(prod - prod.max())
            ##xmax = total.max()
            ##total = np.exp(X[j] * w.T - xmax)
            total_sum = total.sum()
            label = np.zeros(C)
            label[y[j]] = 1
            w = w - step_size * np.dot (np.array (  [( total/total_sum - label)] ).T , [X[j]]) 
