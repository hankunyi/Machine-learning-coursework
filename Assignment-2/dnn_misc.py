...
class linear_layer:
    ...
    def forward(self, X):
        ...
        X = np.hstack( (np.ones((N,1)) , np.mat(X))  )

        forward_output = np.array ( X * np.vstack(( np.mat(self.params['b']) , np.mat(self.params['W'])     ) ) )
    ...
    def backward(self, X, grad):
        ...
        backward_output = np.array (np.mat(grad) * np.mat(self.params['W']).T)
        self.gradient['W'] = np.mat(X).T * np.mat(grad)
        self.gradient['b'] = np.mat(np.ones(N)) * np.mat(grad)
...
class relu:
    ...
    def backward(self, X, grad):
        ...
        X [X>0] =1
        X [X<0] =0
        backward_output = np.multiply(X, grad)
        return backward_output
