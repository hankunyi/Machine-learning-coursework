...
def objective_function(X, y, w, lamb):
    ...
    N, D = X.shape
    reg = lamb / 2 * (np.dot(w.T, w)[0][0])
    error =  1- np.multiply (y, np.dot(X, w).T[0])
    error_sum = 0
    for i in range(error.size):
        if error[i] > 0:
            error_sum = error_sum + error[i]
    obj_value = reg + error_sum / N
    return obj_value
...
def pegasos_train(Xtrain, ytrain, w, lamb, k, max_iterations):
    ...
    for iter in range(1, max_iterations + 1):
        ...
        Aplus = np.multiply (y, np.dot(X, w).T[0])
        learn_rate = 1 / (lamb*iter)
        error = 0
        for i in range (Aplus.size):
            if (Aplus[i] < 1):
                error = error + y[i] * np.array([X[i]]).T
        w = (1 - learn_rate * lamb) * w + learn_rate / k * error
        w = np.minimum(1, 1/np.sqrt(lamb)/np.sqrt(np.dot(w.T, w)[0][0]) ) * w
...
def pegasos_test(Xtest, ytest, w, t = 0.):
    ...
    pred = np.dot(Xtest, w)
    for i in range (N):
        if ((pred[i] >= t and ytest[i] == 1) or (pred[i] <t and ytest[i] == -1)):
            correct = correct + 1
